...
import { getExpenseDetails, postExpense, putExpense } from "../../api/ExpenseApiCalls";
...
class ExpenseForm extends React.Component {
    ...
    async componentDidMount() {
        if (this.state.mode === FormStatus.EDIT) {
            try {
                const res = await getExpenseDetails(this.state.id)
                const res2 = await getExpenseCategories()
                this.setState({
                    isLoaded: true,
                    data: res.data,
    ...
    handleSubmit = async (event) => {
        ...
        if (isValid) {
            if (this.state.mode === FormStatus.NEW) {
                await postExpense(this.state.data)
                navigate(-1, { replace: true })
            } else {
                await putExpense(this.state.data, this.state.id)
                navigate(-1, { replace: true })
            }
        }
    ...
    render() {
        ...
        const pageTitle = mode === FormStatus.NEW ? 'Add expense' : `Edit expense`
        const { navigate } = this.props
        let content;
