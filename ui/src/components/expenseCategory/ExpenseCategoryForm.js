...
import { getExpenseCategory, postExpenseCategory, putExpenseCategory } from "../../api/ExpenseCategoryApiCalls";

class ExpenseCategoryForm extends React.Component {
    constructor(props) {
        super(props);
    ...
    async componentDidMount() {
        if (this.state.mode === FormStatus.EDIT) {
            try {
                const res = await getExpenseCategory(this.state.id)
                this.setState({
                    isLoaded: true,
                    data: res.data
                });
    ...
    handleSubmit = async (event) => {
        ...
        if (isValid) {
            if (this.state.mode === FormStatus.NEW) {
                await postExpenseCategory(this.state.data.Name)
                navigate(-1, { replace: true })
            } else {
                await putExpenseCategory(this.state.data.Name, this.state.id)
                navigate(-1, { replace: true })
            }
        }
    }
...
export default withRouter(ExpenseCategoryForm);